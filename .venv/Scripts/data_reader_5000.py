...
df = pd.read_excel("C:/Users/Veniamin/PycharmProjects/Keyio_Collection/.venv/Scripts/keio_collectionsheet-1.xlsx")
    ...
    for x in range(16):
        for y in range(12):
            if str(plate.iloc[x,y]) != 'nan' and x%2==0:
                new_plate, new_row, new_column = empty_val_assignment(plate.iloc[x, y], plate.iloc[x+1, y], new_plates, 16, 12)
                row_list.append([i+1, chr(ord('@')+int(x/2+1)), y+1, [chr(ord('@')+int(x/2+1)), y+1], plate.iloc[x, y], plate.iloc[x+1, y], new_plate+1, chr(ord('@')+int(new_row/2+1)), new_column+1, [chr(ord('@')+int(new_row/2+1)), new_column+1]])

    print('Plate ' + str(i))
    i += 1

combined_list = pd.DataFrame(row_list, columns=['orig_plate', 'orig_row', 'orig_col', 'orig_col_row', 'keio_name', 'genotype', 'new_plate', 'new_row', 'new_col', 'new_col_row'])
